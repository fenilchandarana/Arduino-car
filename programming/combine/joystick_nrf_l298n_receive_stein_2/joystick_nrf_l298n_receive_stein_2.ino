...
const Motor left_motor(enA, in1, in2);
...
void controlMotors(int speed_joy, int turn_joy) {
  ...
  int turn = mapValue(turn_joy);
  Motor::Direction dir =
    speed == 0 ? Motor::OFF :
    speed > 0 ? Motor::FORWARD :
    Motor::BACKWARD;
  left_motor.setDirection(dir);
  right_motor.setDirection(dir);
  if (turn > 0) {
    left_motor.setSpeed(speed);
    right_motor.setSpeed(max(0, speed - turn));
  }
  else {
    right_motor.setSpeed(speed);
    left_motor.setSpeed(max(0, speed + turn));
  }
}
